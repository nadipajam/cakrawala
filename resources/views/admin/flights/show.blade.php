        ...
        <article class="admin-hero-card">
            ...
            <div class="mt-5 grid gap-4 xl:grid-cols-[1.35fr_.95fr]">
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div class="admin-info-card"><p class="admin-info-label">Flight Number</p><p class="admin-info-value">{{ $flight->flight_number }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Airline</p><p class="admin-info-value">{{ $flight->airline?->name }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Airplane</p><p class="admin-info-value">{{ $flight->airplane?->model }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Price</p><p class="admin-info-value">Rp{{ number_format((float) $flight->price, 0, ',', '.') }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Departure</p><p class="admin-info-value">{{ $flight->departureAirport?->code }} - {{ $flight->departure_time?->format('d M Y H:i') }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Arrival</p><p class="admin-info-value">{{ $flight->arrivalAirport?->code }} - {{ $flight->arrival_time?->format('d M Y H:i') }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Booked Seats</p><p class="admin-info-value">{{ $bookedSeatIds->count() }}</p></div>
                    <div class="admin-info-card"><p class="admin-info-label">Available Seats</p><p class="admin-info-value">{{ $availableSeats }}</p></div>
                </div>
